...
export default class UiMusic {
  ...
  constructor (_option: IUiMusicOption) {
    ...
    if (!(UiMusic.instance instanceof UiMusic)) {
      UiMusic.instance = this
    }
  }
  /** 设置播放进度 */
  ...
  public load (src?: string) {
    ...
    each(setting, (value, key) => isDef(value) && (audio[key] = value))
    return this
  }
  /** 播放 */
  public play = () => {
  ...
  private _handleEvent (eventId: number, event?: any) {
    ...
    if (!isMounted) {
      ...
      append($loading, loading || SvgWindmill)
